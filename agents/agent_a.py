...
from backend.linters import run_sandboxed_linters
from storage.database import get_config
...
def run(repo_name: str, pr_number: int, developer: str, dev_profile: str = "") -> dict:
    ...
    linter_issues = run_sandboxed_linters(diff, config.get("enabled_linters"))
    llm_issues = analyze_diff(diff, developer, dev_profile)
    for item in llm_issues:
        item.setdefault("source", "llm")
    issues = linter_issues + llm_issues
