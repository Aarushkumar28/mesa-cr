from storage.database import get_patterns, save_comment, init_db, mark_clean_review
...
def record_comments(username: str, repo: str, pr_number: int, comments: list):
    ...
    for item in comments:
        seen_categories.append(item.get("category", "general"))
        save_comment(
            username,
            repo,
            pr_number,
    ...
    mark_clean_review(username, seen_categories)
